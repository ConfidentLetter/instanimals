...
async function handlePanelAuth(mode) {
  ...
  const showErr = (msg) => {
    ...
    errEl.classList.add("panel-error");
  };
  ...
  try {
    ...
    const username = isSignup
      ? document.getElementById("panel-username")?.value.trim() ||
        email.split("@")[0]
      : undefined;
    const body = isSignup
...
let posts = [
  {
    id: 1,
    poster: "John_Nature",
    text: "Caught this little fox taking a nap in the woods today. So peaceful!",
    media:
      "https://images.unsplash.com/photo-1517683201413-571216591730?auto=format&fit=crop&w=1200&q=80",
    likes: 128,
    location: "California, USA",
    comments: [{ user: "NatureLens", text: "Amazing shot!" }],
  },
  {
    id: 2,
    poster: "Happy_Paws",
    ...
    text: "Max is a 2-year-old Golden mix waiting for his forever home! ❤️",
    media:
  ...
  },
  {
    id: 3,
    poster: "WildLens",
    text: "A stunning morning at the lake — spotted this heron right at sunrise. Nature never disappoints. 🌅",
    media:
      "https://images.unsplash.com/photo-1444464666168-49d633b86797?auto=format&fit=crop&w=1200&q=80",
    likes: 340,
    location: "Oregon, USA",
    comments: [{ user: "BirdWatch22", text: "What a capture!" }],
  },
  {
    id: 4,
    poster: "San_Jose_Animal_Care",
    isShelter: true,
    website: "sanjoseanimals.org",
    text: "Meet Luna! This 3-year-old tabby is looking for a quiet home. She loves cozy blankets and afternoon naps 🐱",
    media:
  ...
  },
  {
    id: 5,
    poster: "TrailPhoto",
    text: "Found this little turtle crossing the trail today. Took my time and waited for it to pass safely 🐢",
    media:
      "https://images.unsplash.com/photo-1437622368342-7a3d73a34c8f?auto=format&fit=crop&w=1200&q=80",
    likes: 512,
    location: "Texas, USA",
    comments: [{ user: "NatureKid", text: "So cute 🐢" }],
  },
];
...
const contacts = [
  {
    ...
    lastMsg: "Thanks for your inquiry!",
    avatar: "Happy",
    online: true,
  },
];
...
function handleFaviconError(img) {
  ...
  img.insertAdjacentHTML(
    "afterend",
    '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="var(--accent)" width="22" height="22"><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7H4z"/></svg>',
  );
}
...
function submitComment(postId) {
  ...
  posts
    .find((p) => p.id === postId)
    .comments.push({ user: state.myProfile.name, text });
  input.value = "";
  renderApp();
...
function sendChat() {
  ...
  const history =
    chatHistory[state.activeChatId] || (chatHistory[state.activeChatId] = []);
  history.push({
    sender: "me",
...
function renderApp() {
  ...
  headerTitle.innerText = state.activeChatId
    ? "Chat"
    : state.isLoggedIn
      ? titles[state.activeTab]
      : "Instanimals";
  headerLeft.innerHTML =
    state.isLoggedIn && (state.activeTab !== "explore" || state.activeChatId)
    ...
    switch (state.activeTab) {
      case "explore":
        if (!animalsLoaded) {
          ...
          contentArea.innerHTML =
            '<div style="text-align:center; padding:40px; color:var(--muted); font-weight:700;">No animals found. Try again later.</div>';
        } else {
          animals.forEach((a) =>
            contentArea.insertAdjacentHTML("beforeend", createAnimalCard(a)),
          );
        }
        break;
      case "adopt":
        renderAdoptView(contentArea);
        break;
      case "search":
        renderSearchView(contentArea);
        break;
      case "notifications":
      ...
      case "friends":
        state.activeChatId
          ...
          : renderFriendsListView(contentArea);
        break;
      case "profile":
        renderProfileView(contentArea);
        break;
      case "edit-profile":
        renderEditProfileView(contentArea);
        break;
    }
  }
  ...
  document
    ...
    .forEach((link) =>
      link.classList.toggle("active", link.id.includes(state.activeTab)),
    );
  document
...
function createPostCard(p) {
  ...
  const displayName = p.isShelter
    ? formatShelterName(p.poster)
    : `@${p.poster}`;
  let avatarContent;
  if (p.isShelter) {
    if (p.website) {
...
function createAnimalCard(a) {
  ...
  const ttsText =
    ...
    [
      a.name,
      a.breeds ? `a ${a.breeds}` : null,
      a.age,
      a.gender ? `${a.gender.toLowerCase()}` : null,
    ]
      .filter(Boolean)
      .join(", ") + ". Looking for a loving home!";
...
function renderAdoptView(c) {
  ...
  const mapQuery = savedLoc
    ? `animal shelters near ${savedLoc}`
    : "animal shelters";
...
function renderSearchView(c) {
  ...
  input.oninput = () => {
    ...
    posts
      ...
      .forEach((p) => list.insertAdjacentHTML("beforeend", createPostCard(p)));
    if (window.lucide) lucide.createIcons();
  };
  input.focus();
}
...
function renderFriendsListView(c) {
  if (!state.isLoggedIn) {
    c.innerHTML = `<p style="text-align:center; padding:40px">Please login.</p>`;
    return;
  }
...
function handleEditSave() {
  ...
  if (!n.trim()) return;
  state.myProfile.name = n;
  state.myProfile.bio = b;
  localStorage.setItem("user_profile", JSON.stringify(state.myProfile));
  switchTab("profile");
}
...
async function loadAnimals() {
  try {
    ...
    const res = await fetch("/api/adoptable-animals", {
