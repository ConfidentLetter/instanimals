...
async function init(){
  ...
  if(!petId){
    ...
    document.getElementById("error").textContent = "Missing petId. Example: /pet.html?petId=...";
    return;
  }

  const pet = await loadPet(petId);
  document.getElementById("petName").textContent = pet.name || "Unnamed";
  document.getElementById("petMeta").textContent = `${pet.species||""} • ${pet.breed||""} • ${pet.size||""} • ${(pet.ageMonths??"")}mo`;
  document.getElementById("petStatus").textContent = pet.status || "—";
  document.getElementById("petEnergy").textContent = (pet.energy != null) ? `${pet.energy}/5` : "—";
  document.getElementById("petMedical").textContent = pet.medicalNeeds ? "yes" : "no";
  ...
  ["hasYard","hoursPerWeek","experienceLevel","prefersSize"].forEach(id=>{
