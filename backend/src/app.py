...
def compute_match(pet: dict, user: dict) -> dict:
    ...
    if medical and exp <= 1:
        score -= 14
        warnings.append("Pet may need medical care; consider more experience or support.")
...
def require_role(expected: str) -> str:
    ...
    role = get_role(uid)
    if role != expected:
        raise PermissionError(f"Forbidden: requires role={expected}, but you are role={role}")
    return uid
...
def apply_for_pet(pet_id: str):
    try:
        ...
        if pet.get("status") not in (None, "adoptable"):
            return fail("pet not accepting new applications", 409, {"petStatus": pet.get("status")})
        ...
        summary = {
            "name": (f"{step1.get('firstName','')} {step1.get('lastName','')}".strip() if step1 else ""),
            "city": step1.get("city") if step1 else "",
            "email": step1.get("email") if step1 else "",
            "phone": step1.get("phone") if step1 else "",
        }

        user_app_ref = db.collection("users").document(user_uid).collection("applications").document(app_id)
        pet_inbox_ref = db.collection("pets").document(pet_id).collection("applications").document(app_id)
...
def list_pet_applications(pet_id: str):
    try:
        ...
        limit = clamp(int_or_none(request.args.get("limit")) or 50, 1, 50)
        q = (
            db.collection("pets").document(pet_id)
...
def review_application(pet_id: str, app_id: str):
    try:
        ...
        user_app_ref = db.collection("users").document(user_id).collection("applications").document(app_id)

        batch = db.batch()
        batch.set(inbox_ref, {"status": new_status, "updatedAt": firestore.SERVER_TIMESTAMP}, merge=True)
        batch.set(user_app_ref, {"status": new_status, "updatedAt": firestore.SERVER_TIMESTAMP}, merge=True)

        if new_status == "approved":
            batch.set(pet_ref, {"status": "pending", "updatedAt": firestore.SERVER_TIMESTAMP}, merge=True)
